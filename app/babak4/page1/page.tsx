...
export default function Babak4Page1Page() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const timerKeys = [
        'babak4_timer_expiration',
        'babak4_timer_paused_time'
      ];
  ...
  const handleProceed = () => {
    if (typeof window !== 'undefined') {
      localStorage.removeItem('babak4_page1_timer_expiration');
      localStorage.removeItem('babak4_page1_timer_paused_time');
    }
    router.push('/babak4/page2_narration');
  };

  if (isValidating) {
    return (
      <div className="babak4-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div className="babak4-column-right">
           <div className="babak4-options-container">
              {stepConfig.options.map((opt) => {
                ...
                if (isSelected) {
                  if (isAnswerCorrect) {
                    btnClass += " babak4-correct-option";
                  } else if (isAnswerCorrect === false) {
                    btnClass += " babak4-incorrect-option";
                  }
                }
